...
// == 引入首页底部导航路由页面
import HotPage from '../hot/index';
import TrendingPage from '../trending/index';
import FavoritePage from '../favorite/index';
import MemberPage from '../member/index';

// == util
import { EVENT_TYPE_BOTTOM_TAB_CHANGE } from '../../util/EventType';
...
// == 底部导航容器
class AppBottomTabNavigator extends React.Component {
    constructor(props) {
        super(props);
    ...
    render() {
        ...
        return (
            <NavigatorContainer onNavigationStateChange={(prevState, newState, action) => {
                // == 发送一个事件 - tab 发生切换的时候
                DeviceEventEmitter.emit(EVENT_TYPE_BOTTOM_TAB_CHANGE, {
...
class TabBarComponent extends React.Component {
    ...
    // == 如果不指定在 iOS 上默认使用 TabBarBottom ，在 Android 平台上默认使用 TabBarTop 。
