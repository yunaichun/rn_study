...
class Index extends React.Component {
    ...
    _setDynamicTabs () {
        ...
        // == 优化
        // == 1、存在 tab 不会重复创建【但是会发现一个问题 - 列表 TrendingPage 也不会刷新了】
        // == 2、主题发生变化会刷新
        // == 3、tab 被修改会刷新
        if (!this.NavigatorContainer || theme !== this.theme || !ArrayUtil.isEqual(this.preKeys, this.props.keys)) {
            const tabs = {};
            const { trending } = this.props;
            // == 保存上一次的主题色和动态 tab
            this.preTabs = trending;
            this.theme = theme;
