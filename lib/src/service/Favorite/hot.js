...
export default class FavoriteHotStore {
    constructor() {
        this.key = 'hot';
    }
    ...
    getKeys() {
        return new Promise((resolve, reject) => {
            AsyncStorage.getItem(this.key, (error, result) => {
                if (!error) {
                    try {
                        console.log('%c === favorite start ===', 'color: #0f0;');
                        console.log(this.key, JSON.parse(result));
                        console.log('%c === favorite end ===', 'color: #0f0;');
...
export const favoriteHotStore = new FavoriteHotStore();
